...
app.post('/wishlist', function(req, res) {
    ...
    connection3.query(sql1, [BookID], (err, result) => {
      if (err) {
        ...
        const sql2 = "INSERT INTO wishlistTable (BookID, BookTitle, Price) VALUES (?, ?, ?)";
        connection3.query(sql2, [BookID, BookTitle, Price], (err, result) => {
          if (err) {
            ...
            /* I think this should also work not 100%
            const sql3 = "SELECT * FROM wishlistTable"; // the SQL query to select all books from the wishlist table
        connection2.query(sql3, function(error, results, fields) {
        if (error) throw error;
        let wishlistItems = '';
        let totalWishlistPrice = 0; // initialize the total price to zero
        results.forEach(function(row) {
        wishlistItems += `
            <h2>${row.Book}</h2>
            <p>Price: $${row.Price}</p>
         ...
         `;
        totalWishlistPrice = totalWishlistPrice+row.Price; // add the price of each book to the total price
  });
});*/
          }
        });
